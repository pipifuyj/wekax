 ...
 *
 * Built on class for boosting a classifier using Freund & Schapire's Adaboost 
 ...
 */
public class QBoost extends EnsembleClassifier 
  ...
  protected int m_MaxIterations = 15;
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    if (data.classAttribute().isNumeric()) {
      throw new UnsupportedClassTypeException("QBoost can't handle a numeric class!");
    }
    if (m_Classifier == null) {
  ...
  public String toString() {
    ...
    if (m_NumIterations == 0) {
      text.append("QBoost: No model built yet.\n");
    } else if (m_NumIterations == 1) {
      text.append("QBoost: No boosting possible, one classifier used!\n");
      text.append(m_Classifiers[0].toString() + "\n");
    } else {
      text.append("QBoost: Base classifiers and their weights: \n\n");
      for (int i = 0; i < m_NumIterations ; i++) {
  ...
  public static void main(String [] argv) {
    ...
    try {
      System.out.println(Evaluation.evaluateModel(new QBoost(), argv));
    } catch (Exception e) {
