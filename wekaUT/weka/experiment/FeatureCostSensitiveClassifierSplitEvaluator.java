...
/*
 *    FeatureCostSensitiveClassifierSplitEvaluator.java
 ...
 * costs.  Also incorporates different feature costs. This is useful
 * for Active Feature Acquistion.
 *
 * @author Prem Melville
 */
public class FeatureCostSensitiveClassifierSplitEvaluator 
  extends CostSensitiveClassifierSplitEvaluator { 
   ...
   * current directory 
   */
    protected File m_OnDemandDirectory = new File(System.getProperty("user.dir")+"/costs");
  ...
  public Object [] getResult(Instances train, Instances test) 
    ...
    //DEBUG - write similar code to acquire feature costs from files
    //For now assume equal costs
    //  double []featureCosts = new double[train.numAttributes() - 1];
    //      for(int i=0;i<featureCosts.length;i++) featureCosts[i]=1.0;
    
    Evaluation eval = new Evaluation(train, costMatrix);
    //DEBUG - also pass the feature costs to eval
    
    if(m_Classifier instanceof BudgetedLearner){
	((BudgetedLearner) m_Classifier).setFeatureCosts(m_FeatureCosts);
    }
  ...
  public String toString() {
    ...
    String result = "FeatureCostSensitiveClassifierSplitEvaluator: ";
    if (m_Classifier == null) {
...
} // FeatureCostSensitiveClassifierSplitEvaluator
