...
public class ActiveFeatureAcquisitionCVResultProducer 
    implements ResultProducer, OptionHandler, AdditionalMeasureProducer {
    
    /** Indices for range of non-local features (to be acquired) */
    protected int m_NonLocalStartIndex = 15; 
    protected int m_NonLocalEndIndex = 39;
    
    /** Ablational level */
    protected double m_AblationLevel = -1;
    
    /** Select features for ablation randomly */
    protected boolean m_RandomAblation = false;
    ...
    /**
     * Get the value of m_RandomAblation.
     * @return value of m_RandomAblation.
     */
    public boolean getRandomAblation() {
	return m_RandomAblation;
    }
    
    /**
     * Set the value of m_RandomAblation.
     * @param v  Value to assign to m_RandomAblation.
     */
    public void setRandomAblation(boolean  v) {
	m_RandomAblation = v;
    }
    

    public void setAblationLevel(double abLevel){
	m_AblationLevel = abLevel;
    }

    public double getAblationLevel(){
	return m_AblationLevel;
    }
	...
	if(m_AblationLevel>=0 && !m_RandomAblation){
	    int numAtts = m_Instances.numAttributes()-1;
	    m_NonLocalStartIndex = (int) (m_AblationLevel * numAtts);
	    if(m_NonLocalStartIndex<1) m_NonLocalStartIndex=1;//atleast one feature should be available to begin with
	    m_NonLocalEndIndex = numAtts-1;//exclude the class attribute
	    assert (numAtts==m_Instances.classIndex());
	}
	...
	for (int fold = 0; fold < m_NumFolds; fold++) {//For each fold
	    ...
	    Instances global = new Instances(fullTrain,0);//only global instances
	    Instances local = new Instances(fullTrain);//only local instance
	    HashMap localToGlobal = new HashMap();//map local instances to global instances
	    //only copy header and allocate space for training set
	    Instances train=new Instances(fullTrain,fullTrain.numInstances());
	    boolean firstPoint = true;
	    int prevSize = 0;
	    Instances test = runInstances.testCV(m_NumFolds, fold);
	    int pointNum = 0;
	    
	    ablateFeatures(local, run*100+fold);//replace non local features with missing
	    //associate local instances with global instances
	    for(int index=0; index<fullTrain.numInstances(); index++){
		localToGlobal.put(local.instance(index),fullTrain.instance(index));
	    }
    ...
    * instance from another with different settings (ignoring
	...
	setRandomAblation(Utils.getFlag('R', options));
	...
	String result = "ActiveFeatureAcquisitionCVResultProducer: ";
	...
	ActiveFeatureAcquisitionCVResultProducer rp = new ActiveFeatureAcquisitionCVResultProducer();
