...
public class SemiSupCrossValidationResultProducer 
	...
	for (int fold = 0; fold < m_NumFolds; fold++) {
		...
		if (m_ResultListener.isResultRequired(this, key)) {
			...
			//Begin - PM
			Instances unlabeledSet = new Instances(train, m_CurrentSize, train.numInstances()-m_CurrentSize);
			removeLabels(unlabeledSet);
			Object [] seResults = ((SemiSupSplitEvaluator) m_SplitEvaluator).getResult(trainSubset, unlabeledSet, test);
			//End - PM
