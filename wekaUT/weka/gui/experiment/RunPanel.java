...
public class RunPanel extends JPanel implements ActionListener {
  ...
  class ExperimentRunner extends Thread implements Serializable {
    ...
    public void run() {
      ...
      try {
	//====================  BEGIN EDIT mbilenko ============================ 
	File progressFile = File.createTempFile("progress", ".dat", new File("/tmp/"));
	...
	if (!(m_ExpCopy instanceof RemoteExperiment)) {
	  ...
	  while (m_RunThread != null && m_ExpCopy.hasMoreIterations()) {
		...
		+ " Run=" + (m_ExpCopy.getCurrentRunNumber());
	      ...
	      PrintWriter progressOut = new PrintWriter(new BufferedOutputStream(new FileOutputStream(progressFile, true)));
	      progressOut.println(fmt.format(new Date()) + "\t" + current);
	      progressOut.close();
	      //====================  END EDIT mbilenko ============================== 
