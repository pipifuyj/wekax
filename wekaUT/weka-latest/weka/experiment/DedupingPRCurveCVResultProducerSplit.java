...
public class  DedupingPRCurveCVResultProducerSplit
  ...
  protected String m_separateTrainingFile = new String("");
  ...
  public void setAdditionalMeasures(String [] additionalMeasures) {
    ...
    if (m_splitEvaluator != null) {
      System.err.println(" DedupingPRCurveCVResultProducerSplit: setting additional "
			 +"measures for "
			 +"split evaluator");
  ...
  public double getMeasure(String additionalMeasureName) {
	...
	getMeasure(additionalMeasureName);
      ...
      throw new IllegalArgumentException("DedupingPRCurveCVResultProducerSplit: "
  ...
  public void doRun(int run) throws Exception {
    ...
    // If a separate training file is used, create separate folds for it
    ArrayList sepFoldList = null;
    if (m_separateTrainingFile.length() > 0) {
      Instances trainInstances = new Instances(new BufferedReader(new FileReader(m_separateTrainingFile)));
      trainInstances.setClassIndex(trainInstances.numAttributes() - 1);
      trainInstances.randomize(new Random(run));
      sepFoldList = createFoldList(trainInstances, m_numFolds);
      System.out.println("Got separate training file " + m_separateTrainingFile + " of " + trainInstances.numInstances() + " instances");
    } 

    for (int fold = 0; fold < m_numFolds; fold++) {
      Instances train = ((sepFoldList == null) ?
	getTrainingFold(foldList, fold) :
	getTrainingFold(sepFoldList, fold));
